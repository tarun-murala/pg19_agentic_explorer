...
class AgentOrchestrator:
    ...
    def handle_query(self, request: ChatQueryRequest) -> ChatQueryResponse:
        ...
        analyzer_output, analyzer_step = self._run_analyzer(request)
        trace.append(analyzer_step)

        rag_output, rag_step = self._run_rag(request, analyzer_output)
        trace.append(rag_step)

        kg_output, kg_step = self._run_kg(rag_output)
        trace.append(kg_step)

        answer_output, answer_step = self._run_answer(request, analyzer_output, rag_output, kg_output)
    ...
    def stream_query(self, request: ChatQueryRequest):
    ...
    def _run_analyzer(self, request: ChatQueryRequest):
    ...
    def _run_rag(self, request: ChatQueryRequest, _analyzer_output) -> tuple:
        start = datetime.utcnow()
    ...
    def _run_kg(self, rag_output) -> tuple:
        start = datetime.utcnow()
    ...
    def _run_answer(self, request, analyzer_output, rag_output, kg_output):
        start = datetime.utcnow()
